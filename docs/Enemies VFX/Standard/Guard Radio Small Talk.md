...
[Radio Dispatch.wwu](Downloads/Radio%20Dispatcher.wwu)
...
![ExampleImage](img/Radio%20Dispatch%20Events.png)

`NPC_G_Radio_Dispatcher`

`NPC_G_Radio_Dispatcher_02`

`NPC_G_Radio_Dispatcher_STOP`
...
![DispatchContainers](img/Radio%20Dispatch%20Container.png)
...
A window will open on import settings. For this you want to import it as a ==Sound Voice== into the Voice Line named container, and ==Sound SFX== into the radio on/off named containers. Everything else can be left on its default setting.
...
Go ahead and do this to the other "_02" event, unless you have another container to play for it.

For the event that's labeled "_STOP", it's slightly different. It needs to have the same container you selected for both it's similarly named event, and have the type be "Stop" instead of "Play". You can change it in this field.
